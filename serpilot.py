...
def main():
    st.set_page_config(page_title="AI Content Factory V1", page_icon=None, layout='centered',
                       initial_sidebar_state='expanded')
    st.title("AI Content Factory V1")
    ...
    if st.button("Generate Articles"):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(f"{temp_dir}/article_batch_{timestamp}.zip", "rb") as f:
                bytes = f.read()
                b = BytesIO(bytes)
                st.download_button("Download Generated Articles", b, f"article_batch_{timestamp}.zip",
                                   "application/zip")
